  ...
  describe("Stake", function () {
    describe("ShouldPass", function () {
      it("Should stake", async function () {
        const { stakingcontract, token, vani, minStakingPeriod, datafeed } =
        ...
        const stakePosition = await stakingcontract.getStakePositions(
          vani.address
        );
        ...
        expect(stakePosition.length).to.equal(1);
        expect(stakePosition[0].endTime).to.equal(startTime + minStakingPeriod);
        expect(stakePosition[0].ETHAmount).to.equal(stakingValue);
        expect(stakePosition[0].ERC20Amount).to.equal(amount);
      });
      it("Should create multiple positions", async function () {
        ...
        expect(stakePosition.length).to.equal(2);
        expect(stakePosition[0].endTime).to.equal(
          startTimeFirstPosition + minStakingPeriod
        );
        expect(stakePosition[1].endTime).to.equal(
  ...
  describe("Unstake", function () {
    describe("ShouldPass", function () {
      it("Should unstake", async function () {
        ...
        const stakePositionsLengthBefore = (
          await stakingcontract.getStakePositions(vani.address)
        ).length;
        ...
        expect(await token.totalSupply()).to.equal(tokenSupplyBefore - amount);
        expect(
          (await stakingcontract.getStakePositions(vani.address)).length
        ).to.equal(stakePositionsLengthBefore - 1);
      });
      it("Should unstake multiple positions", async function () {
        ...
        expect(await token.totalSupply()).to.equal(
          tokenSupplyBefore - amountOne - amountTwo
        );
      });
      it("Should emit proper event", async function () {
        const { stakingcontract, token, vani, minStakingPeriod } =
        ...
        expect(await stakingcontract.connect(vani).unstake(0))
          ...
          .withArgs(vani.address, amount, 0, amount);
      });
    });
    describe("ShouldFail", function () {
      ...
      it("Should fail if staking position is closed", async function () {
        ...
        await expect(
          stakingcontract.connect(vani).unstake(2)
        ).to.be.revertedWithCustomError(
          stakingcontract,
